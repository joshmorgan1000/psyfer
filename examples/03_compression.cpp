...
void example_fpc_compression() {
    ...
    // Smooth sensor data (compresses well)
    {
        ...
        auto compressed = compression::fpc_compress(sensor_data);
        ...
        size_t count = compression::fpc_decompress(compressed, decompressed);
    ...
    // Financial data (high precision required)
    {
        ...
        auto compressed = compression::fpc_compress(prices);
    ...
    // Different compression levels
    {
        ...
        for (auto level : {compression::fpc_compression_level::MIN,
                          compression::fpc_compression_level::DEFAULT,
                          compression::fpc_compression_level::MAX}) {
            auto compressed = compression::fpc_compress(data, level);
...
void example_multidimensional_fpc() {
    ...
    // Compress using FPC
    auto compressed = compression::fpc_compress_2d(
    ...
    auto tensor_compressed = compression::fpc_compress_tensor(std::span<const double>(temperature_grid), dims);
    ...
    // Decompress and verify
    auto decompressed_result = compression::fpc_decompress_tensor<double>(tensor_compressed);
